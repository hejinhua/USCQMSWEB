...
layui.define(['layer', 'laytpl', 'upload'], function(exports) {
    ...
    return $.ajax({
      ...
      , success: function(res) {
        res.code == 0
          ...
          : layer.msg(res.msg || ((tips || 'Error') + ': 获取数据失败'), {
            time: 5000
          })
  ...
  var layimClose, popmin = function(content) {
    ...
    return layer.open({
      ...
      ,
      resize: false
      //lwp修改过(面板最小化时的头像，获取自己的)
      ,
      content: '<img src="' + (cache.mine.avatar) + '"><span>' + (content || cache.base.title || '我的LayIM') + '</span>'
  ...
  var sendMessage = function() {
    var data = {
      ...
      //lwp修改过的(发送者头像)
      , avatar: `api/src/user/getAvatar/` + cache.mine.id
      ...
      if (othis.data('down')) {
        ...
        events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
          ...
          , success: function(layero) {
            ...
            //lwp增加
            const initGroupFormData = {}
            const GroupFormData = {
              implclass: 'com.usc.app.ims.config.action.group.GetGroupUsers',
              itemNo: 'SUSER',
              userName: localStorage.getItem('userName'),
              otherParam: { groupId: thatChat.data.id }
            }
            initGroupFormData.values = JSON.stringify(GroupFormData)
            members.data = initGroupFormData
            ...
            post(members, function(res) {
              ...
              layui.each(res, function(index, item) {
                li += '<li data-uid="' + item.id + '"><a href="javascript:;"><img src="' + item.avatar + '"><cite>' + item.name + '</cite><cite>' + item.status + '</cite></a></li>'
              ...
              //获取群员
              othis.find('.layim-chat-members').html(res.members || (res || []).length + '人')
              // othis.find('.layim-chat-members').html(res.members || (res.list || []).length + '人')

              //lwp群面板只供查看群成员不支持点击成员进行私聊
              //私聊
