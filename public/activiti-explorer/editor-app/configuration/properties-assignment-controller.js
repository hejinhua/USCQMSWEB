  ...
  function($scope, $http) {
    ...
    $scope.getAllRoles = function(successCallback) {
      $http({
        ...
        url: ACTIVITI.CONFIG.contextRoot + '/getAllRole'
      }) //此处是获取所需角色的URL
        .then(
          function successCallback(reponse) {
            ...
            for (var i = 0; i < obj.length; i++) {
              if (i == 0) {
                initId = obj[i].id + ''
                $scope.getAllAccountByRole(initId) //initId即为userId
              }
    ...
    $scope.getAllAccountByRole = function(roleId) {
      $http({
        method: 'get',
        headers: { Accept: 'application/json', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
        url: ACTIVITI.CONFIG.contextRoot + '/getRoleUserList/' + roleId
      }) //根据roleId获取当前组的用户数据的URL,value为roleId
        .then(
          function successCallback(reponse) {
            //封装数据
            var obj = reponse.data
            if (obj != null) {
              var accounts = []
              for (var i = 0; i < obj.length; i++) {
                accounts.push({ id: obj[i].id, name: obj[i].name, code: obj[i].code, index: i })
              }
    ...
    $scope.save = function() {
      if ($scope.choseAssignmentFlag == 'assignee') {
        ...
        for (var i = 0; i < choseAssignees.length; i++) {
          if (choseAssignees[i].id == choseAssignee) {
            $scope.$emit('choseAssigneeStr', choseAssignees[i].name)
          }
        }
        ...
        for (var i = 0; i < choseAssignees.length; i++) {
          if (choseAssignees[i].selected) {
            choseAssigneesStr += choseAssignees[i].name + ',' //值id改为name
          }
        }
...
var KisBpmChoseCandidateGroupsCtrl = [
  '$scope',
  '$http',
  function($scope, $http) {
    var candidateGroups = []
    $scope.getAllRoles = function(successCallback) {
      $http({
        ...
        function successCallback(reponse) {
          var obj = reponse.data
          for (var i = 0; i < obj.length; i++) {
            candidateGroups.push({ id: obj[i].id, name: obj[i].name })
          }
    ...
    $scope.save = function() {
      ...
      for (var i = 0; i < choseCandidateGroups.length; i++) {
        if (choseCandidateGroups[i].selected) {
          choseCandidateGroupsStr += choseCandidateGroups[i].name + ','
        }
      }
